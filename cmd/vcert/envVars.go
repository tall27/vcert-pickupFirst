...
func assignEnvVarsToFlags(c *cli.Context) {
	colorYellow := "\033[33m"
	colorReset := "\033[0m"
	...
	for _, item := range envVarList {
		...
		if c != nil {
			flagName := strings.TrimLeft(item.FlagName, "-")
			isExplicitlySet = c.IsSet(flagName)
		} else {
			isExplicitlySet = *item.Destination != ""
		}
