...
func TestConvertServerPolicyToInternalPolicy_PkixParameterSet(t *testing.T) {
	...
	t.Run("a single ED25519 curve is allowed", func(t *testing.T) {
		sp := newServerPolicy(true, []string{"1.3.6.1.4.1.28783.10.3.25519"})
		p, err := sp.toPolicy()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		...
		k := p.AllowedKeyConfigurations[0]
		// checkKey validates an ED25519 key against an ECDSA AllowedKeyConfiguration, so the
		// configuration has to be spelled this way for an ED25519 request to be permitted.
		if k.KeyType != certificate.KeyTypeECDSA {
			t.Fatalf("expected ECDSA, got %v", k.KeyType)
		}
		if len(k.KeyCurves) != 1 || k.KeyCurves[0] != certificate.EllipticCurveED25519 {
			t.Fatalf("expected [ED25519], got %v", k.KeyCurves)
		}
	})
