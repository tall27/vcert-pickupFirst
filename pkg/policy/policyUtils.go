...
// ED25519 is an Edwards curve rather than an ECDSA one, but TPP groups it with the elliptic curves,
// and vcert follows suit: certificate.AllSupportedCurves includes it and endpoint.checkKey validates
// an ED25519 key against an ECDSA AllowedKeyConfiguration.
var TppEllipticCurves = []string{"P256", "P384", "P521", "ED25519"}
...
var KeyAlgorithmsToPKIX = map[string]map[string]string{
	...
	"ECC": {
		...
		// ED25519 sits on its own arc rather than under the ECDSA one, because TPP treats it as a
		// distinct algorithm; it is keyed under ECC here so that it decodes to a curve name that
		// certificate.EllipticCurve.Set understands.
